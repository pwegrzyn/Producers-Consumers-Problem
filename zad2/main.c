...
void consume(void)
{
    if(main_buffer[last_delete_index] == NULL)
    {
        if(options->l_mode == FULL)
...
{   
    ...
    if(sem_init(&fill_sem, 0, 0) != 0)
    {
        perror("Error while initializing the fill semaphore");
        exit(EXIT_FAILURE);
    }

    if(sem_init(&empty_sem, 0, options->array_size) != 0)
    {
        perror("Error while initializing the empty semaphore");
        exit(EXIT_FAILURE);
    }
    if(sem_init(&mutex_sem, 0, 1) != 0)
    {
        perror("Error while initializing the mutex semaphore");
        exit(EXIT_FAILURE);
    }
